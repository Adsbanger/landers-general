...
(() => {
    ...
    const GAME_CLICKER_TIME = readAppConfig().secondsLeftBeforeFinal || 10;
    ...
    async function updateTranslationWithParams(element, key, params) {
        if (!element) return;
        const translations = await getTranslations(loadFallbackTranslation, designLocalePath);
    ...
    function onModalWindowClose(e) {
        ...
        if ((_a = config.tabUnderClick) == null ? void 0 : _a.currentTab) {
    ...
    function init() {
        ...
        const stepGame = getCurrentStepFromURL(URL_PARAMS.step, false);
    ...
    document.addEventListener("DesignContentLoaded", (event) => {
        var _a;
        const customEvent = event;
        designLocalePath = (_a = customEvent.detail) == null ? void 0 : _a.localePath;
        init();
    });
    init();
})();